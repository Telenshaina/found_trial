...
const TransactionScreenYield: React.FC<Props> = ({ route }) => {
  ...
  const checkAndUpdateItemStatus = async () => {
    ...
    if (data.yielder_confirmed && data.owner_confirmed) {
  ...
  useEffect(() => {
    const fetchConfirmationStatus = async () => {
      ...
      if (data) {
        setIsReturnConfirmed(data.yielder_confirmed);
        setIsReceivedConfirmed(data.owner_confirmed);
      }
    };
...
export default TransactionScreenYield;